...
def total_visit_last_24_hours(collection):
    ...
    user_count_last_24_hours = (
        result_last_24_hours[0]["user_count_last_24_hours"]
        if result_last_24_hours
        else 0
    )
    user_count_previous_24_hours = (
        result_previous_24_hours[0]["user_count_previous_24_hours"]
        if result_previous_24_hours
        else 0
    )
...
def total_visit_last_7_days(collection):
    ...
    user_count_last_7_days = (
        result_last_7_days[0]["user_count_last_7_days"] if result_last_7_days else 0
    )

    user_count_previous_7_days = (
        result_previous_7_days[0]["user_count_previous_7_days"]
        if result_previous_7_days
        else 0
    )
...
def hourly_visits_last_24h(collection):
    ...
    formatted_result = [
        {"hour": entry["_id"]["hour"], "total_traffic": entry["total_traffic"]}
        for entry in result_hourly_visits
    ]
...
def calculate_daily_visits_for_last_7d(collection):
    ...
    pipeline_daily_visits_last_7_days = [
        ...
        {"$sort": {"_id": 1}},
        {"$project": {"_id": 0, "date": "$_id", "total_visits": 1}},
    ]
...
def busiest_hour_7_days(collection):
    ...
    pipeline = [
        ...
        },
        {
            "$group": {
                "_id": "$_id.date",
                "hourlyVisits": {"$push": {"hour": "$_id.hour", "visits": "$visits"}},
            }
        },
        {"$project": {"_id": 0, "date": "$_id", "hourlyVisits": 1}},
        {"$sort": {"date": 1}},
        {
            "$group": {
                "_id": None,
                "result": {"$push": {"k": "$date", "v": "$hourlyVisits"}},
            }
        },
    ...
    if len(result) > 0:
        for key, value in result[0].items():
            ...
            for hour_visit_dict in value:
                modified_hour = {}
                modified_hour.update(
                    {
                        "hour": (hour_visit_dict["hour"] + 3) % 24,
                        "visits": hour_visit_dict["visits"],
                    }
                )
...
def gender_trend_30_days(collection):
    ...
    daily_visit_trend = [
        ...
        for entry in result_last_30_days
    ]

    # Create a JSON response
    json_response = {"genderTrend30Days": daily_visit_trend}
...
def gender_trend_last_7_weeks(collection):
    ...
    # Execute the aggregation
    result_last_7_weeks_gender = list(
        collection.aggregate(pipeline_last_7_weeks_gender)
    )
    ...
    # Create a JSON response
    json_response = {"genderTrendLast7Weeks": weekly_gender_visit_trend}
...
def gender_trend_12_months(collection):
    ...
    # Execute the aggregation
    result_last_12_months_gender = list(
        collection.aggregate(pipeline_last_12_months_gender)
    )
    ...
    # Create a JSON response
    json_response = {"genderTrendLast12Months": monthly_gender_visit_trend}
...
def monthly_visitors_count(collection):
    ...
    # Extract the results
    total_visitors_last_12_months = [
        {
            "year": entry["_id"]["year"],
            "month": entry["_id"]["month"],
            "total_visitors": entry["total_visitors"],
        }
        for entry in result_last_12_months
    ]

    # Create a JSON response
    json_response = {"totalVisitsLast12Months": total_visitors_last_12_months}
...
def gender_trend_last_7_hours(collection):
    ...
    # Create a JSON response
    json_response = {"genderTrendLast7Hours": gender_distribution_last_7_hours}
    ...
    for i in json_response["genderTrendLast7Hours"]:
        ...
        temp.update(
            {
                "hour": (i["hour"] + 3) % 24,
                "total_male": i["total_male"],
                "total_female": i["total_female"],
                "total_kids": i["total_kids"],
            }
        )
